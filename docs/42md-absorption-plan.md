...
| 项 | 值 |
|---|---|
| **官网** | https://42md.cc/cli |
| **安装/更新命令** | `curl -fsSL https://42md.cc/mac \| bash` |
| **当前版本** | v0.7.28（截至 2026-06-06） |
| **文件大小限制** | 免费版 30MB，Pro 版 100MB |
...
-q, --quality      AI 识别模式（扫描件/精准还原，消耗配额）
--pages <PAGES>    仅转换指定页码范围（如 1-1000 / 1,3,5-8）
-w, --workers      并发数（默认自动选择）
```

**默认模式**：直接提取（原生支持，不消耗配额）
**Quality 模式**：AI OCR（扫描件/精准还原，消耗配额）
